...
class Preprocessing():
    ...
    def preprocess_tlr(self, target: str, is_train = True) -> pd.DataFrame:
    ...
    def prepocess_coge(self):
        """
        Esegue la creazione di lag sul DataFrame.
        :return: DataFrame preprocessato.
    ...
    # def pre_predict()
    def predict(self, df: pd.DataFrame, start:str, horizon_window: int, df_is_preprocessed = False) -> pd.Series:
        
        # if not df_is_preprocessed:
        #     inner_prep = Preprocessing(df,self.dict_features)
        #     df = inner_prep.preprocess_tlr(self.target)
        y_pred = []
        max_hour_back = max(self.time_shifts)
        ...
        ind = df.loc[pd.to_datetime(start):pd.to_datetime(start) + pd.Timedelta(hours = horizon_window)].index
        ...
        for i in ind:
            ...
            for var, (n_lag, h_window, M_period) in self.dict_features.items():
                ...
                if var != self.target:
                    ...
                    rolling_window = df[var].rolling(f'{h_window}H', min_periods=h_window)
                    df.loc[i,f'median_rolling_{var}'] = rolling_window.median()[i]
                    df.loc[i,f'dev_std_rolling_{var}'] = rolling_window.std()[i]
                    df.loc[i,f'mean_rolling_{var}'] = rolling_window.mean()[i]
                    df.loc[i,f'max_rolling_{var}'] = rolling_window.max()[i]
                    df.loc[i,f'min_rolling_{var}'] = rolling_window.min()[i]
                    df.loc[i,f'perc_25_rolling_{var}'] = rolling_window.quantile(0.25)[i]
                    df.loc[i,f'perc_75_rolling_{var}'] = rolling_window.quantile(0.75)[i]
...
df_prep = prep.preprocess_tlr('T2M')
...
model, best_params = prep.fit_grid(model_params, X_train, y_train, 2)

path_test = r'C:\Users\Mipper\Desktop\Progetti\NIA-MED\tlr_class_forecasting\Meteo_1Gen_Dic2023_test_predict.csv'
df_test = pd.read_csv(path_test, sep = ';')
df_test.Datetime = pd.to_datetime(df_test.Datetime, format = '%d/%m/%Y %H:%M')
df_test.set_index('Datetime', inplace = True)
prep_test = Preprocessing(df_test, prep.dict_features)
df_test = prep_test.preprocess_tlr(prep.target, is_train = False)

y_pred = prep.predict(df_test,'2023-12-31 00:00',24)


import pickle
path_save = r'C:\Users\Mipper\Desktop\Progetti\NIA-MED\tlr_class_forecasting\model_test.pkl'
with open(path_save, 'wb') as tf:
